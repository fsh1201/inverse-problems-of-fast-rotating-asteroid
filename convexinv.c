/*     A procedure for shape+spin+scattering modelling of lightcurves
	   ...
	   Modified by Shuai Feng, taking exposure time into account
...
int main(int argc, char* argv[])
{
	...
	int i, j, l, m, k, nrows, ndata, k2, ndir,
		* ia, ial0, ial0_abs, ia_beta_pole, ia_lambda_pole, ia_prd, ia_par[4], ia_cl,
		ind_par_file, ind_lc_out_file, ind_out_file, ind_lin_file, ind_lout_file,ind_parin_file,
		arg_shift, verbose, output, onlyrel,
	...
	double** as, ** ae, am1, am2, bm1, bm2, cm1, cm2, escl_m;
	int k22;
	...
	as = matrix_double(MAX_N_OBS, 3);
	ae = matrix_double(MAX_N_OBS, 3);
	...
	ind_lin_file += arg_shift;
	ind_lout_file += arg_shift;
	ind_parin_file += arg_shift;
	...
	for (i = 1; i <= Lcurves; i++)
	{
		...
		for (j = 1; j <= Lpoints[i]; j++)
		{
			...
			//printf("%f %f %f %f %f %f %f %f ", tim[ndata], brightness[ndata], e0[1], e0[2], e0[3], e[1], e[2], e[3]);

			/*fast rotating*/
			fscanf(stdin, "%lf", &fExposuret[ndata]);	//exposure time
			//printf("%f ", fExposuret[ndata]);
			fTimeInterval[ndata] = prd / 200.0 / 24.0;
			//printf("%f ", fTimeInterval[ndata]);
			dSegmentOfExposuret[ndata] = fmax((int)(fExposuret[ndata] / fTimeInterval[ndata] + 0.5), 1);
			fTimeInterval[ndata] = fExposuret[ndata] / dSegmentOfExposuret[ndata];
	...
	sphfunc(Numfac, at, af,Mmax,Lmax, Ts,  Tc, Fs, Fc, Pleg, Dsph);
	/* sph. harm. coeffs. of the initial ell. */
	ellfit(cg, a, b, c, Numfac, Ncoef, at, af,Mmax,Lmax,Pleg);
	...
	{
		fprintf(f_lc_out, "%d %d\n", Lpoints[i], 1 - Inrel[i]);
		/* loop over one lightcurve */
		for (j = 1; j <= Lpoints[i]; j++)
		{
			k++;
			if (Inrel[i] == 1)
				fprintf(f_lc_out, "%f %f %.8f %.8f %.8f %.8f %.8f %.8f\n", tim[k]+jd_0, Yout[k] * Sclnw[i] / (cg[Ncoef + 4] * exp(-alpha[k] / cg[Ncoef + 5]) + cg[Ncoef + 6] * alpha[k] + 1), as[k][1], as[k][2], as[k][3], ae[k][1], ae[k][2], ae[k][3]);
			else
				fprintf(f_lc_out, "%f %f %.8f %.8f %.8f %.8f %.8f %.8f\n", tim[k]+jd_0, Yout[k], as[k][1], as[k][2], as[k][3], ae[k][1], ae[k][2], ae[k][3]);
		}
	}
